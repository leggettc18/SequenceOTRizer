    ...
    if (std::filesystem::exists(customMusicPath)) {
        ...
        for (auto item : list) {
            if (item.path().extension() == ".seq") {
                ...
                if (!std::filesystem::exists(item.path().parent_path() / item.path().stem() += ".ootrs" / item.path().stem() += ".meta")) {
                    std::cerr << item.path().generic_string()
                                << " does not have a corresponding .meta file! Skipping." << std::endl;
                    continue;
                }
                if (std::filesystem::exists(item.path().parent_path() / item.path().stem() += ".ootrs" / item.path().stem() += ".zbank")) {
                    std::cerr << item.path().generic_string()
